...
#                 self.get_init_pos_player = (posx, posy)
#                 break
...
#         '''
#         return self.get_init_pos_player
    
#     def backtracking(self):
#         current = self.get_init_pos_player() #posição inicial do jogador
#         stack = [current] #pilha para armazenar os movimentos
#         visited = [] #lista para armazenar as posições visitadas
...
#             visited.append(point) # adiciona o ponto na lista de visitados

#             # vetor com os pontos vizinhos de point
#             neighbors = [(point[0] - 1, point [1]), (point[0] + 1, point[1]),
#                         (point[0], point[1] - 1), (point[0], point[1] + 1)]
            
#             find = False # Variável auxiliar que informa se foi encontrado algum vizinho livre
...
#             if not find:
#                 stack.pop() # Se não foi encontrado nenhum vizinho livre, remove o topo da pilha

#         return False # Se a pilha estiver vazia, retorna False indicando que não foi encontrado o prêmio
...
import random
import threading
import time # ir preenchendo aos poucos
...
class Maze:
    ...
    def backtracking(self):
        current = self.get_init_pos_player() # seta a posição inicial do jogador
        stack = [current] # Cria a pilha e adiciona a posição inicial do jogador
        visited = [] # Cria a lista de visitados para armazenar as posições visitadas
        ...
        while stack:
            ...
            for neighbor in neighbors:
                if (0 <= neighbor[0] < self.M.shape[0] and 0 <= neighbor[1] < self.M.shape[1] and
                    ...
                    find = True # aqui muda o valor de find indicando que foi encontrado um vizinho livre
                    stack.append(neighbor) # aqui adiciona essa posição no topo da pilha
                    self.mov_player(neighbor) # aqui movimenta o jogador para essa posição
                    time.sleep(0.05) # aqui controla a velocidade dos passos

                    break

            if not find:
                stack.pop() # temos que se não for encontrado um vizinho livre remove o topo da pilha
